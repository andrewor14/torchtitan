...
activation, i.e. w4a4) — the full set of layers an NVFP4 eval quantizes
(everything except ``lm_head`` / ``router`` / ``gate``).
"""
...
def _wrap_linear_nvfp4_fake_quant(module: nn.Linear) -> None:
    ...
    NVFP4 fake-quantized (STE) before the matmul — i.e. w4a4, matching the NVFP4
    eval. Instance-level forward override (eager only)."""
...
def apply_nvfp4_fake_quant(model: nn.Module) -> nn.Module:
    ...
    linear layers (w4a4) — the full set of layers quantized by the NVFP4 eval.
